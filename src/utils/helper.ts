...
const paramsToHex = (params: TxParams) => {
	for (let i in params) {
		if (Array.isArray(params[i])) {
			params[i] = paramsToHex(params[i])
		} else {
...
const sha3 = (datas) => {
	...
	return hash.update(Buffer.from(datas)).digest('hex');
};
...
const defineProperties = (self, fields, data) => {
	...
	self.serialize = function serialize() {
		return Buffer.from(encode(self.raw)).toString('hex');
	};
	...
	if (data) {
		if (typeof data === 'string') {
			data = toBuffer(stripHexPrefix(data));
		}
