...
import { RpcMethod, RpcPayload, TxParams, Block, Transaction, TransactionReceipt } from '../types';
...
const rpcMethod = (rpc: RpcMethod) => {
	...
	return {
		...
		sha3: (params: any) => rpc<string>(payload('web3_sha3', [params])),
		...
		getBalance: (address: string) => rpc<string>(payload('sym_getBalance', [address, helper.DEFAULT_BLOCK]), helper.hexToNumberString),
		accounts: () => rpc<string[]>(payload('sym_accounts')),
		blockNumber: () => rpc(payload('sym_blockNumber', [])),
		getBlockByHash: (blockHash: string, isFull: boolean = false) => rpc<Block>(payload('sym_getBlockByHash', [blockHash, isFull])),
		getBlockByNumber: (blockNumber: string = helper.DEFAULT_BLOCK, isFull: boolean = false) => rpc<Block>(payload('sym_getBlockByNumber', [blockNumber, isFull])),
		...
		sign: (address: string, msg: string) => rpc<string>(payload('sym_sign', [address, msg])),
		getTransactionCount: (address: string, blockNumber: string = helper.DEFAULT_BLOCK) => rpc<string>(payload('sym_getTransactionCount', [address, blockNumber]), helper.hexToNumberString),
		getTransactionByHash: (hash: string) => rpc<Transaction>(payload('sym_getTransactionByHash', [hash])),
		...
		getTransactionReceipt: (hash: string) => rpc<TransactionReceipt>(payload('sym_getTransactionReceipt', [hash])),
		getDeposit: (address: string) => rpc(payload('sym_getDeposit', [address])),

		sendTransaction: (txParams: TxParams, pk: string) => rpc(payload('sym_sendRawTransaction', [tx.sign(txParams, pk)])),
		sendRawTransaction: (txRaw: string) => rpc(payload('sym_sendRawTransaction', [txRaw])),
	};
};
